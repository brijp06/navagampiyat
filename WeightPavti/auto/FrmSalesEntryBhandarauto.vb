...
Public Class FrmSalesEntryBhandarauto
    Dim barc As String
    ...
    Private Sub ButSave_Click(sender As Object, e As EventArgs) Handles ButSave.Click
        ...
        For i As Integer = 0 To Dg.Rows.Count - 1
            ...
            ob.Execute("Insert Into Acstock(Cid, Year_id, Department, Billno, Billdate,Stockin, Stockout, Rate, Basic, CGST, SGST, CGSTAMT, SGSTAMT,ptype,itemid,Sizeid,barcode,billtype) Values(1,'" & clsVariables.WorkingYear & "','" & Cmbdepartment.Text & "'," & Billno.Text & ",'" & ob.DateConversion(billDt.Text) & "',0," & Dg.Rows(i).Cells(4).Value & "," & Dg.Rows(i).Cells(5).Value & "," & Dg.Rows(i).Cells(6).Value & "," & Dg.Rows(i).Cells(7).Value & "," & Dg.Rows(i).Cells(9).Value & "," & Dg.Rows(i).Cells(8).Value & "," & Dg.Rows(i).Cells(10).Value & ",'Sales'," & Val(Dg.Rows(i).Cells(2).Tag) & ",'" & (iid) & "','" & Trim(Dg.Rows(i).Cells(1).Value) & "','" & cmbType.Text & "')", ob.getconnection())
        Next
        ...
        If Val(Roundoff.Text) <> 0 Then
            If Val(Roundoff.Text) > 0 Then
                ...
                ob.Execute("Insert Into Acdata(Cid, Year_id, Type, Docno, Docdate, Acid, ACName, Cramt,Remarks,Dramt,ptype,Department) Values(1,'" & clsVariables.WorkingYear & "','" & cmbType.Text & "'," & Billno.Text & ",'" & ob.DateConversion(billDt.Text) & "'," & Val(dac) & ",N'139',0,N'" & Trim(Remarks.Text) & "'," & Val(Roundoff.Text) & ",'Sales','" & Cmbdepartment.Text & "')", ob.getconnection())

            Else
                Roundoff.Text = Math.Abs(Val(Roundoff.Text))
                ob.Execute("Insert Into Acdata(Cid, Year_id, Type, Docno, Docdate, Acid, ACName, Cramt,Remarks,Dramt,ptype,Department) Values(1,'" & clsVariables.WorkingYear & "','" & cmbType.Text & "'," & Billno.Text & ",'" & ob.DateConversion(billDt.Text) & "'," & Val(dac) & ",N'90',0,N'" & Trim(Remarks.Text) & "'," & Val(Roundoff.Text) & ",'Sales','" & Cmbdepartment.Text & "')", ob.getconnection())
    ...
    Private Sub Billno_Validating(sender As Object, e As System.ComponentModel.CancelEventArgs) Handles Billno.Validating
        Dim dt As DataTable = ob.Returntable("select * from acmain where billno=" & Val(Billno.Text) & " and Billtype='" & Trim(cmbType.Text) & "' and Department='" & Trim(Cmbdepartment.Text) & "' and ptype='Sales'", ob.getconnection())
        If dt.Rows.Count > 0 Then
            If MessageBox.Show("Do You Want To Edit This Entry...?", Application.ProductName, MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) = Windows.Forms.DialogResult.Yes Then
                ...
                Dim dts As DataTable = ob.Returntable("select * from acstock where billno=" & Val(Billno.Text) & " and Billtype='" & Trim(cmbType.Text) & "' and Department='" & Trim(Cmbdepartment.Text) & "' and ptype='Sales'", ob.getconnection())
                If Dg.Rows.Count > 0 Then
                    Dg.Rows.Clear()
                End If
    ...
    Private Sub Barcode_Validating(sender As Object, e As System.ComponentModel.CancelEventArgs) Handles Barcode.Validating
        If Barcode.Text <> "" Then
            ...
            cgst.Text = ob.FindOneString("Select CGST_Per From Item_Master Where item_Name=N'" & Trim(itemname.Text) & "'", ob.getconnection())
            sgst.Text = ob.FindOneString("Select CGST_Per From Item_Master Where item_Name=N'" & Trim(itemname.Text) & "'", ob.getconnection())
            rate.Text = ob.FindOneString("Select MRP From Acstock Where barcode='" & Barcode.Text & "' and ptype='Purchase' order by billno desc", ob.getconnection())
    ...
    Private Sub Button3_Click(sender As Object, e As EventArgs) Handles Button3.Click
        Dim dss As DataTable = ob.Returntable("select billno from acmain where  ptype='sales' and billtype='Credit' and Roundoff<>0 and Department='BHANDAR'", ob.getconnection())
        For n As Integer = 0 To dss.Rows.Count - 1
            Dim dt As DataTable = ob.Returntable("select * from acmain where billno=" & Val(dss.Rows(n).Item("billno")) & " and ptype='sales' and billtype='Credit' and Roundoff<>0 and Department='BHANDAR'", ob.getconnection())
            If dt.Rows.Count > 0 Then
                Billno.Text = Val(dss.Rows(n).Item("billno"))
                'If MessageBox.Show("Do You Want To Edit This Entry...?", Application.ProductName, MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) = Windows.Forms.DialogResult.Yes Then
                billDt.Text = dt.Rows(0).Item("Billdate")
                Cname.Tag = dt.Rows(0).Item("partyid")
                Cname.Text = ob.FindOneString("select Member_name from Member_Master where Member_id=" & dt.Rows(0).Item("partyid") & "", ob.getconnection())
                ...
                tNetamt.Text = dt.Rows(0).Item("Paymentamt")
                basci.Text = dt.Rows(0).Item("Basic")
                Dim dts As DataTable = ob.Returntable("select * from acstock where billno=" & Val(dss.Rows(n).Item("billno")) & " and ptype='sales' and billtype='Credit' and Department='BHANDAR'", ob.getconnection())
                If Dg.Rows.Count > 0 Then
                    Dg.Rows.Clear()
                End If
                For i As Integer = 0 To dts.Rows.Count - 1
