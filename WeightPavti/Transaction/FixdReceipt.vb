...
Public Class FixdReceipt
    ...
    Private Sub MemberInward_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Billno.Text = ob.FindOneString("select isnull(max(Billno),0)+1 from Acmain where ptype='FixdReceipt'", ob.getconnection())
        acname.Tag = 100
    ...
    Private Sub ButSave_Click(sender As Object, e As EventArgs) Handles ButSave.Click
        ob.Execute("delete from acmain where billno=" & Val(Billno.Text) & " and ptype='Fixdreceipt' and year_id='" & clsVariables.WorkingYear & "'", ob.getconnection())
        ob.Execute("delete from acdata where Docno=" & Val(Billno.Text) & " and ptype='Fixdreceipt' and year_id='" & clsVariables.WorkingYear & "'", ob.getconnection())

        ob.Execute("Insert Into Acmain(Cid, Year_id, Department, Billtype, Billno, Billdate, PartyId, Acid, Remarks, Netamt,ReceiptAmt,ptype, Per, Period, Duedate,fdno,clid,cbj) Values(" & clsVariables.CompnyId & ",'" & clsVariables.WorkingYear & "','Fixd'," & aq(cmbtype.Text) & "," & Val(Billno.Text) & ",'" & ob.DateConversion(billDt.Text) & "'," & Val(sname.Tag) & "," & Val(acname.Tag) & ",N'" & Trim(Txtremark.Text) & "'," & Val(Netamt.Text) & "," & Val(Netamt.Text) & ",'FixdReceipt'," & percent.Text & "," & Period.Text & ",'" & ob.DateConversion(duedt.Text) & "'," & Val(Fixno.Text) & "," & Val(Columnname.Tag) & "," & Val(payac.Tag) & ")", ob.getconnection())
        If Trim(cmbtype.Text) = "CASH" Then
            ob.Execute("Insert Into Acdata(Cid, Year_id, Type, Docno, Docdate, Acid, ACName,cramt ,Remarks,dramt,ptype) Values(1,'" & clsVariables.WorkingYear & "','" & cmbtype.Text & "'," & Billno.Text & ",'" & ob.DateConversion(billDt.Text) & "'," & acname.Tag & ",N'" & acname.Text & "'," & Netamt.Text & ",N'" & Trim(Txtremark.Text) & "',0,'FixdReceipt')", ob.getconnection())
            ob.Execute("Insert Into Acdata(Cid, Year_id, Type, Docno, Docdate,Acid, ACName, dramt,Remarks,Cramt,ptype) Values(1,'" & clsVariables.WorkingYear & "','" & cmbtype.Text & "'," & Billno.Text & ",'" & ob.DateConversion(billDt.Text) & "'," & payac.Tag & ",N'" & payac.Text & "'," & Netamt.Text & ",N'" & Trim(Txtremark.Text) & "',0,'FixdReceipt')", ob.getconnection())
        Else
            ' ob.Execute("Insert Into Acdata(Cid, Year_id, Type, Docno, Docdate, Acid, ACName,dramt ,Remarks,cramt,ptype) Values(1,'" & clsVariables.WorkingYear & "','" & cmbtype.Text & "'," & Billno.Text & ",'" & ob.DateConversion(billDt.Text) & "'," & acname.Tag & ",N'" & acname.Text & "'," & Netamt.Text & ",N'" & Trim(Txtremark.Text) & "',0,'FixdReceipt')", ob.getconnection())
            'ob.Execute("Insert Into Acdata(Cid, Year_id, Type, Docno, Docdate,Acid, ACName, cramt,Remarks,dramt,ptype) Values(1,'" & clsVariables.WorkingYear & "','" & cmbtype.Text & "'," & Billno.Text & ",'" & ob.DateConversion(billDt.Text) & "'," & payac.Tag & ",N'" & payac.Text & "'," & Netamt.Text & ",N'" & Trim(Txtremark.Text) & "',0,'FixdReceipt')", ob.getconnection())
            ob.Execute("Insert Into Acdata(Cid, Year_id, Type, Docno, Docdate, Acid, ACName,cramt ,Remarks,dramt,ptype) Values(1,'" & clsVariables.WorkingYear & "','" & cmbtype.Text & "'," & Billno.Text & ",'" & ob.DateConversion(billDt.Text) & "'," & acname.Tag & ",N'" & acname.Text & "'," & Netamt.Text & ",N'" & Trim(Txtremark.Text) & "',0,'FixdReceipt')", ob.getconnection())
            ob.Execute("Insert Into Acdata(Cid, Year_id, Type, Docno, Docdate,Acid, ACName, dramt,Remarks,Cramt,ptype) Values(1,'" & clsVariables.WorkingYear & "','" & cmbtype.Text & "'," & Billno.Text & ",'" & ob.DateConversion(billDt.Text) & "'," & payac.Tag & ",N'" & payac.Text & "'," & Netamt.Text & ",N'" & Trim(Txtremark.Text) & "',0,'FixdReceipt')", ob.getconnection())
        End If
    ...
    Private Sub Billno_KeyDown(sender As Object, e As KeyEventArgs) Handles Txtremark.KeyDown, Period.KeyDown, percent.KeyDown, payac.KeyDown, sname.KeyDown, Netamt.KeyDown, duedt.KeyDown, cmbtype.KeyDown, Billno.KeyDown, billDt.KeyDown, acname.KeyDown, Fixno.KeyDown, Columnname.KeyDown
    ...
    Private Sub TextBox3_Validated(sender As Object, e As EventArgs) Handles Period.Validated
        ...
        duedt.Text = DateAndTime.DateAdd(DateInterval.Day, Val(Period.Text), dat)
    End Sub
    ...
    Private Sub Billno_Validating(sender As Object, e As System.ComponentModel.CancelEventArgs) Handles Billno.Validating
        Dim dt As DataTable = ob.Returntable("select * from acmain where billno=" & Val(Billno.Text) & " and ptype='Fixdreceipt' and year_id='" & clsVariables.WorkingYear & "'", ob.getconnection())
    ...
    Private Sub Txtremark_Validating(sender As Object, e As CancelEventArgs) Handles Txtremark.Validating
