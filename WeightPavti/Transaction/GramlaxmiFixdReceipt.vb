...
Public Class GramlaxmiFixdReceipt
    ...
    Private Sub MemberInward_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Billno.Text = ob.FindOneString("select isnull(max(Billno),0)+1 from Acmain where ptype='GramlaxmiFixdReceipt'", ob.getconnection())
        acname.Tag = 24
    ...
    Private Sub ButSave_Click(sender As Object, e As EventArgs) Handles ButSave.Click
        ob.Execute("Insert Into Acmain(Cid, Year_id, Department, Billtype, Billno, Billdate, PartyId, Acid, Remarks, Netamt,ReceiptAmt,ptype, Per, Period, Duedate,fdno) Values(" & clsVariables.CompnyId & ",'" & clsVariables.WorkingYear & "'," & aq(cmbtype.Text) & "," & aq(cmbtype.Text) & "," & Val(Billno.Text) & ",'" & ob.DateConversion(billDt.Text) & "'," & Val(sname.Tag) & "," & Val(acname.Tag) & ",N'" & Trim(Txtremark.Text) & "'," & Val(Netamt.Text) & "," & Val(Netamt.Text) & ",'GramlaxmiFixdReceipt'," & percent.Text & "," & Period.Text & ",'" & ob.DateConversion(duedt.Text) & "'," & Val(Fixno.Text) & ")", ob.getconnection())
        ob.Execute("Insert Into Acdata(Cid, Year_id, Type, Docno, Docdate, Acid, ACName,cramt ,Remarks,dramt,ptype) Values(1,'" & clsVariables.WorkingYear & "','" & cmbtype.Text & "'," & Billno.Text & ",'" & ob.DateConversion(billDt.Text) & "'," & acname.Tag & ",N'" & acname.Text & "'," & Netamt.Text & ",N'" & Trim(Txtremark.Text) & "',0,'GramlaxmiFixdReceipt')", ob.getconnection())
        ob.Execute("Insert Into Acdata(Cid, Year_id, Type, Docno, Docdate,Acid, ACName, dramt,Remarks,Cramt,ptype) Values(1,'" & clsVariables.WorkingYear & "','" & cmbtype.Text & "'," & Billno.Text & ",'" & ob.DateConversion(billDt.Text) & "'," & payac.Tag & ",N'" & payac.Text & "'," & Netamt.Text & ",N'" & Trim(Txtremark.Text) & "',0,'GramlaxmiFixdReceipt')", ob.getconnection())
    ...
    Private Sub Columnname_Validated(sender As Object, e As EventArgs) Handles Columnname.Validated
        If Columnname.Text <> "" Then
            ...
            Columnname.Text = ob.FindOneString("Select Column_Name From Column_Master Where  Column_Id=" & Val(Columnname.Tag) & "", ob.getconnection())
        End If
        If Val(Columnname.Tag) = 43 Then
            Period.Text = 108

        Else
            MessageBox.Show("Wrong Column")
            Columnname.Clear()
            Columnname.Focus()
            Columnname.Tag = 0
        End If
