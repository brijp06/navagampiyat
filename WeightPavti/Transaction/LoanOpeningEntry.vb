...
Public Class LoanOpeningEntry
    ...
    Private Sub ButSave_Click(sender As Object, e As EventArgs) Handles ButSave.Click
        Dim acname As String = ob.FindOneString("Select Account_Id from Column_Master where Column_Id=" & Val(Columnname.Tag) & "", ob.getconnection())
        ...
        ob.Execute("Insert Into PASSING_DATA(Loan_no, Doc_Date, member_id, File_no, Period, Percentage, Due_Date, Column_Id, Amount, Jamin_1_Id, Jamin_2_Id) values(" & Val(Billno.Text) & "," & aq(ob.DateConversion(billDt.Text)) & "," & Val(sname.Tag) & "," & Val(Loanno.Text) & "," & Val(Period.Text) & "," & Val(percent.Text) & "," & aq(ob.DateConversion(duedt.Text)) & "," & Val(Columnname.Tag) & "," & Val(Netamt.Text) & "," & Val(Jamin1.Tag) & "," & Val(Jamin2.Tag) & ")", ob.getconnection())
        ...
        Next
        ob.Execute("Insert Into Acmain(Cid, Year_id, Department, Billtype, Billno, Billdate, PartyId, Acid, Remarks,PaymentAmt,intamt,Clid,Loanno,ptype) Values(" & clsVariables.CompnyId & ",'" & clsVariables.WorkingYear & "','Opening','Opening'," & Val(Billno.Text) & ",'" & ob.DateConversion(billDt.Text) & "'," & Val(sname.Tag) & "," & Val(acname) & ",N'" & Trim(sname.Text) & "'," & Val(Netamt.Text) & "," & Val(intamt.Text) & "," & Val(Columnname.Tag) & "," & Val(Loanno.Text) & ",'LoanOpening')", ob.getconnection())
    ...
    Private Sub Billno_KeyDown(sender As Object, e As KeyEventArgs) Handles Period.KeyDown, percent.KeyDown, sname.KeyDown, Netamt.KeyDown, duedt.KeyDown, Columnname.KeyDown, Billno.KeyDown, billDt.KeyDown, Jamin1.KeyDown, Loanno.KeyDown, intamt.KeyDown
