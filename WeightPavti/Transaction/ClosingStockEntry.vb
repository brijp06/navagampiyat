...
Public Class ClosingStockEntry
    Private Sub JVEntry_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Docno.Text = ob.FindOneString("select isnull(max(Billno),0)+1 from Acmain where cid=" & Val(clsVariables.CompnyId) & " and ptype='ClosingStock' and Year_id='" & clsVariables.WorkingYear & "'", ob.getconnection())
    ...
    Private Sub Docno_Validating(sender As Object, e As System.ComponentModel.CancelEventArgs) Handles Docno.Validating
        ...
        dt = ob.Returntable("Select * from Acdata where Docno=" & Val(Docno.Text) & " and PType='ClosingStock' and year_id='" & clsVariables.WorkingYear & "' and Tid=" & Val(Docno.Text) & "", ob.getconnection())
        If dt.Rows.Count > 0 Then
            ...
            If result1 = 6 Then
                For i As Integer = 0 To dt.Rows.Count - 1
                    ...
                    dg.Rows(dg.Rows.Count - 1).Cells(1).Tag = ob.FindOneString("select Partyid from Acmain where Billno=" & Val(Docno.Text) & " and Billtype='ClosingStock' and acid=" & Val(dt.Rows(i).Item("acid")) & "", ob.getconnection())
    ...
    Private Sub ButSave_Click(sender As Object, e As EventArgs) Handles ButSave.Click
        If Val(ddr.Text) = Val(dcr.Text) Then
            ob.Execute("Delete from Acmain where year_id='" & clsVariables.WorkingYear & "' and ptype='ClosingStock' and Billno=" & Docno.Text & " and cid=" & Val(clsVariables.CompnyId) & "", ob.getconnection())
            ob.Execute("Delete from Acdata where year_id='" & clsVariables.WorkingYear & "' and ptype='ClosingStock' and Docno=" & Docno.Text & " and cid=" & Val(clsVariables.CompnyId) & "", ob.getconnection())
            For i As Integer = 0 To dg.Rows.Count - 1
                ob.Execute("Insert Into Acdata(Cid, Year_id, Type, Docno, Docdate, Acid, ACName, Cramt,Remarks,Dramt,ptype,tid) Values(" & clsVariables.CompnyId & ",'" & clsVariables.WorkingYear & "','Transfer'," & Docno.Text & ",'" & ob.DateConversion(Ddate.Text) & "'," & dg.Rows(i).Cells(0).Tag & ",N'" & dg.Rows(i).Cells(0).Value & "'," & dg.Rows(i).Cells(3).Value & ",N'" & Trim(dg.Rows(i).Cells(2).Value) & "'," & dg.Rows(i).Cells(4).Value & ",'ClosingStock'," & Docno.Text & ")", ob.getconnection())
                ob.Execute("Insert Into Acmain(Cid, Year_id, Department, Billtype, Billno, Billdate, PartyId, Acid,Receiptamt,Paymentamt,Remarks,ptype) Values(" & clsVariables.CompnyId & ",'" & clsVariables.WorkingYear & "','ClosingStock','ClosingStock'," & Val(Docno.Text) & ",'" & ob.DateConversion(Ddate.Text) & "'," & Val(dg.Rows(i).Cells(1).Tag) & "," & Val(dg.Rows(i).Cells(0).Tag) & "," & Val(dg.Rows(i).Cells(3).Value) & "," & Val(dg.Rows(i).Cells(4).Value) & ",N'" & Trim(dg.Rows(i).Cells(2).Value) & "','ClosingStock')", ob.getconnection())
            Next
            'open()
    ...
    Public Sub open()
        For i As Integer = 0 To dg.Rows.Count - 1
            If Val(dg.Rows(i).Cells(3).Value) <> 0 Then
                Dim clid As String = ob.FindOneString("select opid from Closingstock where clid=" & Val(dg.Rows(i).Cells(0).Tag) & "", ob.getconnection())
                Dim dsdate As Date = Ddate.Text
                dsdate = dsdate.AddDays(1)
                ob.Execute("Insert Into Acdata(Cid, Year_id, Type, Docno, Docdate, Acid, ACName, dramt,Remarks,cramt,ptype) Values(" & clsVariables.CompnyId & ",'" & clsVariables.WorkingYear & "','Transfer'," & Docno.Text & ",'" & ob.DateConversion(dsdate) & "'," & dg.Rows(i).Cells(0).Tag & ",N'" & dg.Rows(i).Cells(0).Value & "'," & dg.Rows(i).Cells(3).Value & ",N'" & Trim(dg.Rows(i).Cells(2).Value) & "'," & dg.Rows(i).Cells(4).Value & ",'ClosingStock')", ob.getconnection())
                ob.Execute("Insert Into Acdata(Cid, Year_id, Type, Docno, Docdate, Acid, ACName, cramt,Remarks,dramt,ptype) Values(" & clsVariables.CompnyId & ",'" & clsVariables.WorkingYear & "','Transfer'," & Docno.Text & ",'" & ob.DateConversion(dsdate) & "'," & Val(clid) & ",N'" & dg.Rows(i).Cells(0).Value & "'," & dg.Rows(i).Cells(3).Value & ",N'" & Trim(dg.Rows(i).Cells(2).Value) & "'," & dg.Rows(i).Cells(4).Value & ",'ClosingStock')", ob.getconnection())
            End If
        Next
    End Sub
    ...
    Private Sub ButDelete_Click(sender As Object, e As EventArgs) Handles ButDelete.Click
        ...
        ob.Execute("Delete from Acdata where year_id='" & clsVariables.WorkingYear & "' and type='ClosingStock' and Docno=" & Docno.Text & " and cid=" & Val(clsVariables.CompnyId) & "", ob.getconnection())
        If dg.Rows.Count > 0 Then
            dg.Rows.Clear()
        End If
        Docno.Text = ob.FindOneString("select isnull(max(Billno),0)+1 from Acmain where cid=" & Val(clsVariables.CompnyId) & " and ptype='ClosingStock' and Year_id='" & clsVariables.WorkingYear & "'", ob.getconnection())
